...
app.get("/barber", (req, res) => {
  ...
  admin
    ...
    .then(async (userData) => {
      ...
      let incomin = user.appointment.filter((obj) => obj.accepted == false);
      let completed = user.appointment.filter((obj) => obj.accepted == false);
      ...
      res.render("barber", { loggedIn: true, user: user });
    })
    .catch((error) => {
      res.redirect("/login");
      console.log(error);
