...
app.post("/appoinment", async (req, res) => {
  ...
  let app = {
    id: data.id,
    time: Date.now(),
    length: parseInt(data.service.split(" - ")[1]),
    name: data.name,
    email: data.email,
    phone: data.phone,
  ...
  const appointment = FirebaseData.createAppointment(app);

  for (let index = 0; index < appointment.length; index++) {
    let time = time + appointment[index].length;
  }

  res.render("confirm", { data: appointment });
});
