...
class FirebaseData {
  ...
  static async createAppointment(data) {
    ...
    if (snapshot.docs[0].data().appointment != undefined) {
      ...
      // for (let index = 0; index < appointments.get().length; index++) {
      //   let tempSnapshot = (await appointments.get()).
      // }

      console.log(await (await appointments.get()).size);

      let id = (await appointments.add({ appointments: storeAppArray })).id;
