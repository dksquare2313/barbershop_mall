...
class FirebaseData {
  ...
  static async createAppointment(data) {
    let snapshot = (
      await db.collection("master").where("_id", "=", data.uid).get()
    ).docs[0];

    let user = db.collection("master").doc(snapshot.id);
    let appList = snapshot.data();

    if (appList.appointment == undefined) {
      user.update({
        appointment: [data],
      });
    } else {
      let tempArray = appList.appointment;

      let check = tempArray.find((obj) => obj.email == data.email);

      if (check == undefined) {
        tempArray.push(data);

        let testData = user.update({
          appointment: tempArray,
        });
  ...
  static async getData(id, uid) {
    let snapshot = (
      await db.collection("master").where("_id", "=", uid).get()
    ).docs[0].data();
