...
class FirebaseData {
  ...
  static async createAppointment(data) {
    let snapshot = (
      await db.collection("master").where("_id", "=", data.id).get()
    ).docs[0];
    ...
    if (appList.appointment == undefined) {
      ...
      let tempArray = appList.appointment.push(data);
      for (let index = 0; index < appList.length; index++) {
        tempArray.push(appList.appointment[index]);
      }
      console.log(appList.appointment.push(data));

      // user.update({
      //   appointment: appList.push(data),
      // });
    }
